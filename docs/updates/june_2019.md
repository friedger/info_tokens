...
ThreeFold has provided a scalable answer to this global challenge, through a global network of decentralized internet capacity that is autonomously managed. This network is supported by independent farmers (capacity providers) and enables developers to easily deploy services on a "self-driving" driving internet grid. This enables current cloud services as well as blockchain protocols to scale beyond the limitations (complexity, security and scalability) of current ICT solutions.

ThreeFold Token is the medium to buy and sell, autonomous and decentralized internet capacity on ThreeFold Grid and is backed by hardware added by farmers on the network.

ThreeFold Token is a token of the future internet, it’s where the internet is headed, closer to the user and to the edge of the network.
